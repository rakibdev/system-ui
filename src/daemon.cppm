...
gboolean onServerEvent(GIOChannel* channel, GIOCondition condition, gpointer) {
...
void onTerminateBySystem(int) { destroy(EXIT_SUCCESS); }
...
void onHttpRequest(int client) {
  ...
  if (!req.starts_with("GET /")) { close(client); return; }
  std::size_t pathEnd = req.find(" HTTP");
  if (pathEnd == std::string::npos) { close(client); return; }

  std::string path = urlDecode(req.substr(5, pathEnd - 5));
  std::size_t space = path.find(' ');
  std::string extName = space != std::string::npos ? path.substr(0, space) : path;
  std::string args = space != std::string::npos ? path.substr(space + 1) : "";

  Extension* ext = nullptr;
  for (auto& [p, e] : manager.extensions) {
    if (p.find("/" + extName + "/") != std::string::npos) { ext = e.get(); break; }
  }
...
void startHttpServer() {
  ...
  g_io_add_watch(httpChannel, G_IO_IN,
                 [](GIOChannel* ch, GIOCondition, gpointer) -> gboolean {
                   int client = accept(g_io_channel_unix_get_fd(ch), nullptr, nullptr);
                   if (client >= 0) std::thread(onHttpRequest, client).detach();
                   return TRUE;
                 },
                 nullptr);
