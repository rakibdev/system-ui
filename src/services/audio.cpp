...
void setDefault(Node* node) {
  std::string key = node->type == NodeType::Sink ? "default.audio.sink"
                                                 : "default.audio.source";
  std::string value = "{\"name\":\"" + node->name + "\"}";
  pw_thread_loop_lock(loop);
  pw_metadata_set_property((pw_metadata*)metadata, 0, key.c_str(),
                           "Spa:String:JSON", value.c_str());
  pw_thread_loop_unlock(loop);
}
  ...
  if (info->change_mask & PW_NODE_CHANGE_MASK_PARAMS) {
    for (size_t index = 0; index < info->n_params; ++index) {
      if (info->params[index].id == SPA_PARAM_Props) {
        pw_node_enum_params((pw_node*)node->proxy, 0, info->params[index].id, 0,
                            -1, nullptr);
        break;
      }
    }
  ...
  if (info->change_mask & PW_DEVICE_CHANGE_MASK_PARAMS) {
    for (size_t index = 0; index < info->n_params; ++index) {
      if (info->params[index].id == SPA_PARAM_Route) {
        pw_device_enum_params((pw_device*)device->proxy, 0,
                              info->params[index].id, 0, -1, nullptr);
        break;
      }
    }
  }
...
void onGlobalObject(void* data, uint32_t id, uint32_t permissions,
  ...
  if (interface == PW_TYPE_INTERFACE_Node) {
    ...
    pw_node_add_listener((pw_node*)node->proxy, &node->listener, &nodeEvents,
                         node.get());
    (isSink ? sinks : sources).emplace_back(std::move(node));
    Log::info("Audio: added " + std::string(isSink ? "sink" : "source") +
              " (total sinks=" + std::to_string(sinks.size()) +
  ...
  if (interface == PW_TYPE_INTERFACE_Device) {
    ...
    if (mediaClass && std::string(mediaClass) == "Audio/Device") {
      ...
      pw_device_add_listener((pw_device*)device->proxy, &device->listener,
                             &deviceEvents, device.get());
