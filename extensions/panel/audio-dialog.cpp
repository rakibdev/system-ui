...
// PipeWire max volume (1.4 = 140% raw, shown as 100% on slider)
constexpr float MAX_VOLUME = 1.4f;
// Safe limit before amplification warning
constexpr float SAFE_LIMIT = 0.6f;
// Slider percentage where safe limit sits
const uint8_t SAFE_PERCENT = std::round(SAFE_LIMIT / MAX_VOLUME * 100);
...
void updateRow(SliderRow& row) {
  ...
  uint8_t sliderValue = std::round(volume / MAX_VOLUME * 100);
  uint16_t displayPercent = std::round(volume / SAFE_LIMIT * 100);
...
SliderRow createSlider(Box* section, Audio::Node* node, bool isActive) {
  ...
  SliderRow result = {node,    sliderPtr, overlay,     percentPtr,
                      namePtr, dotPtr,    containerPtr};
  updateRow(result);
  return result;
}

// AMD onboard audio ignores pipewire volume changes
...
void create() {
...

  {
    ...
    auto settingsButton = std::make_unique<Button>(Button::Type::Icon,
                                                   Button::None, Button::Small);
